...
create table Actor(act_id int Not Null,act_fname char(20),act_lname char(20),
...
create table Genres(gen_id int Not Null,gen_title char(20) Not Null,Primary Key(gen_id));
...
(9017, 'Hannah Steele'),
(9018, 'Vincent Cadena'),
(9019, 'Brand Sponseller'),
(9020, 'Richard Adams');


create table Movie(mov_id int Not Null ,mov_title char(50),mov_year int,mov_time int,mov_lang char(50) Not Null,mov_dt_rel date,mov_rel_country char(50) ,Primary Key(mov_id));
...
create table Rating(mov_id int Not Null ,rev_id int Not Null,rev_stars int,num_o_ratings int,foreign key(mov_id) references movie(mov_id),foreign key(rev_id) references reviewer(rev_id));
...
select mov_title from movie where mov_id in (select mov_id from movie_genres 
...
AND rating.rev_stars>=8 AND reviewer.rev_name IS NOT NULL;
...
SELECT mov_title, mov_year, mov_dt_rel, dir_fname, dir_lname, act_fname, act_lname FROM movie a, movie_direction b, director c, rating d, reviewer e, actor f, movie_cast g WHERE a.mov_id=b.mov_id AND b.dir_id=c.dir_id AND a.mov_id=d.mov_id AND d.rev_id=e.rev_id AND a.mov_id=g.mov_id 
AND g.act_id=f.act_id AND e.rev_name ="Neal Wruck";